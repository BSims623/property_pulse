...
const PropertyCard = ({ property }) => {
    const getRateDisplay = ({ monthly, weekly, nightly }) => {
        if (monthly) return `$${monthly.toLocaleString()}/mo`
        else if (weekly) return `$${weekly.toLocaleString()}/wk`
        else return `$${nightly.toLocaleString()}/night`
    }
            ...
            <div className="p-4">
                ...
                >
                    {Object.keys(property.rates).map((rate) => (
                        <p key={rate} className='capitalize'><FaMoneyBill className='inline mr-2' /> {rate}</p>
                    ))}
                </div>
